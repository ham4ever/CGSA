  ...
  c.querySelectorAll('button[data-action]').forEach(function(btn) {
    ...
    btn.addEventListener('click', function() {
      ...
      if (btn.dataset.action === 'picked') {
        // If answering picked (Ja or Nee), you visited the room
        if (s.visited === null) {
          s.visited = true;
          s.visitedAt = new Date().toISOString();
          s.visitedBy = currentUser;
        }
      ...
      if (card) {
        ...
        // update button states for both visited and picked

        ['visited', 'picked'].forEach(function(act) {
          card.querySelectorAll('button[data-action="' + act + '"]').forEach(function(b) {
      ...
      // Auto-scroll to next pending room ONLY when 'Iets opgehaald?' (picked) is clicked
      ...
      if (btn.dataset.action === 'picked') {
        ...
        if (target) {
          floatRoomIndex = allCards.indexOf(target);
          updateFloatCounter();
          setTimeout(function() {
            target.scrollIntoView({behavior: 'smooth', block: 'center'});
          }, 240);
